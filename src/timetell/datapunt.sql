...
CREATE OR REPLACE VIEW "{schemaname}".v_timetell_contracts_team_month AS
...
--aggregate results on team level (nr. of intern/extern contracts).
,intern_extern_per_team_per_month AS (
    SELECT
	    contract_month,
	    team,
	    SUM(CASE WHEN contractsoort_name = 'Extern' THEN 1 ELSE 0 END) AS num_extern,
	    SUM(
		    CASE WHEN contractsoort_name = 'Intern'
		    THEN 1 ELSE 0 END) AS num_intern
    FROM emp_contracts_per_month
    GROUP BY contract_month, team
)

--Select statement shows per team, per month (untill NOW), nr. of contract types.
SELECT *
